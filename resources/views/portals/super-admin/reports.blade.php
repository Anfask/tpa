...
@section('header_subtitle', 'Generate and extract performance data records')
...
@section('content')
    <div style="display: flex; flex-direction: column; gap: 1.5rem;">
        ...
        <div class="card" style="display: flex; flex-direction: column; gap: 1.25rem;">
            ...
            <p style="font-size: 0.85rem; color: var(--text-secondary);">Filter evaluations and download a structured
                spreadsheet (CSV format).</p>

            <form action="{{ route('super-admin.reports.export') }}" method="GET" target="_blank"
                ...
                <div class="form-group" id="teacher-group">
                    ...
                    <select class="form-control" id="teacher_id" name="entity_id">
                        ...
                        @foreach($teachers as $t)
                            <option value="{{ $t->id }}">{{ $t->name }} ({{ $t->campus->name ?? 'No Campus' }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group" id="admin-group" style="display: none;">
                    <label class="form-label" for="admin_id">Select Administrator (Optional)</label>
                    <select class="form-control" id="admin_id" name="entity_id">
                        <option value="">All Administrators</option>
                        @foreach($admins as $a)
                            <option value="{{ $a->id }}">{{ $a->name }} ({{ $a->campus->name ?? 'No Campus' }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group" id="campus-group" style="display: none;">
                    <label class="form-label" for="campus_id">Select Campus Node (Optional)</label>
                    <select class="form-control" id="campus_id" name="entity_id">
                ...
                <div style="display: flex; gap: 1rem; margin-top: 0.5rem;">
                    <button type="submit" name="format" value="csv" class="btn btn-primary"
                        style="flex: 1; justify-content: center; padding: 0.85rem;">
                        Download CSV
                    </button>
                    <button type="submit" name="format" value="pdf" class="btn btn-accent"
                        style="flex: 1; justify-content: center; padding: 0.85rem;">
                        Download PDF
                    </button>
                </div>
        ...
        <div class="card" style="display: flex; flex-direction: column; gap: 1rem;">
            <h3>Understanding TPA Exports</h3>
            <p style="font-size: 0.9rem; line-height: 1.5; color: var(--text-secondary);">
                The export engine extracts real-time database inputs directly from the MySQL schemas.
            </p>
            <ul
                style="margin-left: 1.25rem; font-size: 0.85rem; color: var(--text-secondary); display: flex; flex-direction: column; gap: 0.5rem;">
                <li><strong>Teacher reports:</strong> Logs scores, dates, inspector names, class context, and total points
                    earned.</li>
                <li><strong>Admin reports:</strong> Logs leadership performance evaluation percentages, target admins, and
                    timestamps.</li>
                <li><strong>Campus reports:</strong> Extracts cleanliness and safety checklists scores logged by admins.
                </li>
            </ul>
            <div
                ...
                Excel and PDF programs (e.g. Microsoft Excel, Google Sheets, Adobe Acrobat) can easily open and import
                standard CSV spreadsheets directly.
            </div>
        </div>
    </div>
