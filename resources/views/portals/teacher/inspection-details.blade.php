...
@section('title', 'Evaluation Detail Summary - TPA')
@section('header_title', 'Evaluation Report Sheet')
@section('header_subtitle', 'Breakdown of evaluation points and criteria scoring')

@section('content')
    <div class="grid-cols-3 animate-fade-in" style="grid-template-columns: 1fr 2fr; gap: 1.5rem;">
        ...
        <div class="card" style="display: flex; flex-direction: column; gap: 1.5rem; height: fit-content;">
            <div style="text-align: center; border-bottom: 1px solid var(--border-color); padding-bottom: 1.5rem;">
                <div
                    style="font-size: 0.85rem; font-weight: 600; color: var(--text-muted); text-transform: uppercase; margin-bottom: 0.5rem;">
                    Evaluation Grade
                </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 1rem;">
                <div>
                    <label
                        style="font-size: 0.75rem; font-weight: 600; color: var(--text-muted); text-transform: uppercase;">
                        Date & Time
                    </label>
                ...
                <div>
                    ...
                    <div style="font-size: 0.8rem; color: var(--text-secondary);">Class:
                        {{ $inspection->campusClass->name ?? 'None' }}</div>
                </div>
            ...
            <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                @foreach($answers as $ans)
                    <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 1.25rem;">
                        <div
                            style="display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem; margin-bottom: 0.5rem;">
                            <div>
                                <span class="badge badge-info"
                                    style="font-size: 0.65rem; margin-bottom: 0.35rem; display: inline-block;">
                            ...
                            <div
                                style="display: flex; align-items: baseline; gap: 0.15rem; font-size: 1.2rem; font-weight: 700; color: var(--primary);">
                                <span>{{ $ans['score'] }}</span>
                        ...
                        @if($ans['comment'])
                            <div
                                style="padding: 0.75rem; border-radius: 8px; background: rgba(var(--text-muted), 0.03); border: 1px solid var(--border-color); font-size: 0.85rem; color: var(--text-secondary); margin-top: 0.5rem;">
