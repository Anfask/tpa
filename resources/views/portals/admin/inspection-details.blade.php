...
@php
    ...
    if ($inspection->type === 'admin') {
        $backUrl = route('admin.performance');
    } elseif ($inspection->type === 'teacher' && isset($inspection->teacher_id)) {
        $backUrl = route('admin.teachers.profile', $inspection->teacher_id);
    }
@endphp
<div style="display: flex; gap: 1rem; align-items: center; margin-bottom: 1.5rem;" class="animate-fade-in">
    <a href="{{ $backUrl }}" class="btn btn-secondary"><i class="fa-solid fa-arrow-left"></i> Back</a>
    <span class="badge badge-info" style="font-size: 0.9rem; padding: 0.4rem 0.8rem;">Log ID: #{{ $inspection->id }}</span>
</div>

<div class="inspection-detail-grid animate-fade-in">
